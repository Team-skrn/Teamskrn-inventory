...
const SUPABASE_URL = 'https://bhaexzjmspamqxcszkgm.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJoYWV4emptc3BhbXF4Y3N6a2dtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTE1NjQ3MjEsImV4cCI6MjA2NzE0MDcyMX0.vCYNKtOp6phSWEK36XYNqL6DznS5pw_49QduBfUvckk';
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // ALWAYS setup event listeners first, regardless of Supabase status
    ...
    if (!supabase) {
        ...
        console.log('App will work in offline mode (login will work, but data won\'t persist)');
    }
    ...
    if (savedUser) {
        try {
            ...
            if (supabase) {
                loadInventoryData().catch(err => console.error('Failed to load inventory:', err));
                loadWishlistData().catch(err => console.error('Failed to load wishlist:', err));
            }
            return;
        } catch (e) {
...
function setupEventListeners() {
    ...
    // Name form submission - CRITICAL FOR LOGIN
    const nameForm = document.getElementById('nameForm');
    console.log('nameForm element:', nameForm);
    ...
    if (nameForm) {
        ...
        // Add fresh listener to the new form
        newForm.addEventListener('submit', function(e) {
            console.log('=== FORM SUBMIT EVENT TRIGGERED ===');
            e.preventDefault();
            e.stopPropagation();
            console.log('Default prevented, calling handleNameSubmit');
            handleNameSubmit(e);
            return false;
        }, true);
        ...
        console.error('ERROR: nameForm not found!');
    }
...
async function loadInventoryData() {
    ...
    if (!supabase) {
        console.error('Cannot load inventory: Supabase not initialized');
